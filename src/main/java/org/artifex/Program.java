...
//        w = new Window("Title");
        ...
        DescriptorBindings bindings = new DescriptorBindings(2,VK_SHADER_STAGE_COMPUTE_BIT)
                .add(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,0,1,Integer.BYTES*100)
                .add(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,1,1,Integer.BYTES*100);
        ...
        DescriptorSet set = new DescriptorSet(device,bindings,1);
         copyWrite = new DescriptorCopyWrite(set,2,1)
        ...
        Semaphore wait = new Semaphore(device);
         signal = new Semaphore(device);

        System.out.println("---------------------------------------------------------");

        for (Long handle : set.getHandles()) {
        ...
        IntBuffer output = MemoryUtil.memIntBuffer(set.getBuffer(1).map(),100);
...
//        }
    ...
    public void record(DescriptorSet set, DescriptorBindings bindings,Compute compute){
        ...
        vkCmdDispatch(commandBuffer.getCommandBuffer(),1,0,0);

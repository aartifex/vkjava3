...
public class DescriptorSet
{
    ...
    public static final DescriptorSet NULL = new DescriptorSet();
    ...
    private DescriptorSet(){
        ...
        this.layout=0L;
        this.pool=0L;
        this.bufferInfo=new ArrayList<>(1);
    }
    ...
    public IntBuffer mapAsInt(int binding,int descriptorNum){
        VulkanBuffer buffer =bindings.getBinding(binding).getBuffer();
        Objects.requireNonNull(buffer,"Buffer was null. Descriptor has not properly been allocated.");

        GLSLayout layout = bindings.getBinding(binding).getGlsLayout();

        IntBuffer b = MemoryUtil.memIntBuffer(buffer.map()+layout.getDescriptor(descriptorNum).getOffset(),
                layout.getDescriptor(descriptorNum).getLength());
        return b;
    }
    ...
    public void setCopyWrite(DescriptorCopyWrite copyWrite) {
        this.copyWrite = copyWrite;
    }

    public DescriptorCopyWrite getCopyWrite() {
        return copyWrite;
    }
    ...
    DescriptorCopyWrite copyWrite = DescriptorCopyWrite.NULL;
